...
const User_fromJSON = (json) => { const r = User_validator.safeParse(JSON.parse(json)); if (r.success) return r.data; throw new JITValidationError(r.issues); };
...
const toPublicUser: (value: { id: number; name: string; email: string; role: "admin" | "member"; active: boolean; score: number; tags: string[]; createdAt: string; profile: { bio: string | null } | undefined }) => { id: number; name: string; role: "admin" | "member" } = /*#__PURE__*/ (() => {
