...
export type AotOutputFormat = "javascript" | "typescript";
...
export interface GenerateOptions {
   ...
   * by the application's TypeScript build. @default "javascript"
   */
  ...
  function emitStandaloneArtifact(name: string, value: unknown, sourceFile: string | undefined): void {
    ...
    const declaredType =
      artifact.kind === "validator"
        ? standaloneType(name, artifact, sourceFile, options.outDir)
        : artifact.kind === "operation"
          ? operationType(name, artifact, sourceFile, options.outDir)
          : sourceFile
...
function buildSubpathModules(
  ...
  for (const [sourceFile, names] of bySource) {
    ...
    if (layout.kind === "package" && layout.format === "javascript") {
...
function writeManifest(
  ...
  const manifestFiles = [
    ...
    ...(layout.kind === "package"
      ? [...(layout.format === "javascript" ? ["index.cjs", "index.d.cts"] : []), "package.json"]
      : []),
    ...modules.flatMap((module) => [
      `${module.name}${layout.jsExtension}`,
      ...(layout.format === "javascript" ? [`${module.name}.d.ts`] : []),
      ...(layout.kind === "package" && layout.format === "javascript"
        ? [`${module.name}.cjs`, `${module.name}.d.cts`]
        : []),
    ]),
  ];
