...
const cases: readonly OperatorCase[] = [
  ...
  { kind: "lowercase", base: JIT.string().lowercase(), custom: JIT.string().lowercase(MESSAGE), invalid: "Ada" },
  { kind: "uppercase", base: JIT.string().uppercase(), custom: JIT.string().uppercase(MESSAGE), invalid: "Ada" },
  { kind: "camelCase", base: JIT.string().camelCase(), custom: JIT.string().camelCase(MESSAGE), invalid: "user_name" },
  {
    kind: "pascalCase",
    base: JIT.string().pascalCase(),
    custom: JIT.string().pascalCase(MESSAGE),
    invalid: "userName",
  },
  { kind: "snakeCase", base: JIT.string().snakeCase(), custom: JIT.string().snakeCase(MESSAGE), invalid: "userName" },
  { kind: "kebabCase", base: JIT.string().kebabCase(), custom: JIT.string().kebabCase(MESSAGE), invalid: "user_name" },
  {
    kind: "upperSnakeCase",
    base: JIT.string().upperSnakeCase(),
    custom: JIT.string().upperSnakeCase(MESSAGE),
    invalid: "user_name",
  },
...
function declaredCheckKinds(): ReadonlySet<string> {
  ...
  for (const match of source.matchAll(/\bkind:\s*"([A-Za-z0-9]+)"/g)) {
