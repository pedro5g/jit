...
Runtime parameters are read from the typed params object. `JIT.cqrs.const(value)`
...
identity and indexed equality. The [entity and index guide](/docs/reference/operators/entity-keyed-and-indexes)
...
plan(users); // eager result array — the builder is the query
plan.to.iterator(); // synchronous iterator
plan.to.asyncIterator(); // async source/backpressure
plan.to.visitor(); // callback sink, returns emitted count
```
...
`JIT.cqrs.query(rowset)` reuses the same condition AST for fixed-offset scans. It
