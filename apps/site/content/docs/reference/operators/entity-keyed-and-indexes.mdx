...
const normalizeUsers = Compiler.compileNormalize(Users.schema);
...
const equalUsers = JIT.compare.equal(IndexedUsers).compile();
...
const normalizeUsers = Compiler.compileNormalize(Users.schema);
const uniqueUsers = Compiler.compileUniqueBy(Users.schema);
```
...
| Return a `Map` from one query execution              | `JIT.cqrs.query(Users).keyed("id")`     |
...
| `Compiler.compileNormalize` default key |             yes |                     yes |                     yes |
| `Compiler.compileUniqueBy` default key  |             yes |                     yes |                     yes |
| `Compiler.compileSortBy` fallback key   |             yes |                     yes |                     yes |
| Validator duplicate-key check           |              no |                      no |                      no |
| Query result memoization                |              no |                      no |                      no |

`JIT.state.watch()` and `JIT.state.watchedList()` receive their key explicitly because
they own change-tracking state rather than compile an equality strategy:

```ts
const members = JIT.state.watchedList(Users, initialUsers, { key: "id" });
