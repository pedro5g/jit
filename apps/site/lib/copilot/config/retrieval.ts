...
/**
 * Broad explanations may retain a lower-scoring conceptual passage when the
 * coverage plan selected one of its source-derived facets. This is not a
 * general low-score escape hatch: only concept/overview entries qualify, and
 * the floor remains above the weakest candidate generated by expansion.
 *
 * A single semantic seed can be much stronger than a second, complementary
 * concept page even when both are useful. Keeping the normal floor for every
 * result collapsed the acceptance question to one passage; this narrower
 * exception preserves coverage without admitting the reference-page noise
 * that the context sweep rejected.
 */
export const MIN_BROAD_FACET_CONFIDENCE = 0.45;
