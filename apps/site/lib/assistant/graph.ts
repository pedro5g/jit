...
export const CONCEPTS: ConceptNode[] = [
  ...
  },
  {
    ...
    mechanisms: [
      ...
      "Sort fields and offset limits are checked before a query plan is formed, so application code receives normalized conditions rather than reparsing transport syntax in a hot query loop. limits.maxDepth bounds declared traversal, pagination.maxOffset bounds a deep page (default 10000), and limits.maxCost is a semantic complexity budget charged while the request is normalized.",
      "JIT.api.authorize(definition, plan, action) returns the effective request: the consumer predicate and the actor predicate joined with and, plus a projection narrowed to both allowlists. A field the API never exposed is rejected; a field the actor cannot read is removed. No access node reaches the adapter, and definition.explain() reports the permitted fields, their cost and the budget.",
    ],
    mechanismsPt: [
      ...
      "Campos de ordenação e limites offset são checados antes de formar um plano de query, então a aplicação recebe condições normalizadas em vez de reprocessar sintaxe de transporte dentro do loop quente. limits.maxDepth limita a profundidade declarada, pagination.maxOffset limita páginas profundas (padrão 10000) e limits.maxCost é um orçamento de complexidade semântica cobrado enquanto a requisição é normalizada.",
      "JIT.api.authorize(definition, plan, action) devolve a requisição efetiva: o predicado do consumidor e o do ator unidos por and, mais uma projeção reduzida às duas allowlists. Um campo que a API nunca expôs é rejeitado; um campo que o ator não pode ler é removido. Nenhum nó de acesso chega ao adapter, e definition.explain() informa campos permitidos, custo e orçamento.",
    ],
    example:
      'const User = JIT.object({ id: JIT.string(), active: JIT.boolean(), score: JIT.number() });\n\nconst activeUsers = JIT.cqrs\n  .query(User)\n  .where((q) => q.eq("active", true))\n  .select("id", "score")\n  .orderBy("score", "desc")\n  .limit(10);\n\nconst Search = JIT.api.query(User, {\n  filter: { active: ["eq"], score: ["gte"] },\n  sort: ["score"],\n  pagination: { type: "offset", defaultLimit: 20, maxLimit: 100, maxOffset: 5_000 },\n  limits: { maxCost: 12 },\n});\nconst parseSearch = JIT.api.parse(Search);\n\nconst readUsers = JIT.access(User).can("read", { fields: ["id", "score"] });\nconst authorizeSearch = JIT.api.authorize(Search, readUsers, "read");',
