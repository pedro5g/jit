...
are canonicalized as filters, ordering, projection, then limit. Repeated
projection/ordering declarations use the last value and repeated limits use
the smallest bound in runtime, AOT, and the descriptor.

`JIT.cqrs.input(...)` exposes the same versioned entry point, but its
definition describes *capabilities* instead of a static request: permitted
