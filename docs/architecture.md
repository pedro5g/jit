...
single-package: `@pedro5g/jit/runtime` exports the runtime `JIT` namespace, and
`@pedro5g/jit/define` exports the same schema DSL with AOT stubs for compiled
...
- `.d.ts` types anchor on the dev's schema file via
  `import("@pedro5g/jit").Infer<typeof import("./user.jit.js").User>` — inference is
...
`jit/user.jit.ts` using `@pedro5g/jit/define`. `jit doctor` reports resolved
