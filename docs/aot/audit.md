...
single-package implementation. The package remains `jit`, with transitional
subpaths that map to the planned split:

- `jit/runtime` maps to the planned runtime host entrypoint.
- `jit/define` maps to the planned AOT definition host entrypoint.
- `jit` keeps the compatibility root namespace.
...
- `jit/define` AOT stubs throw if executed and register artifact metadata for
  `jit generate`.
...
These contracts are now public through `import { Host } from "jit"` and
...
conveniences, so importing `jit/define` is a host-compatible API step but not
